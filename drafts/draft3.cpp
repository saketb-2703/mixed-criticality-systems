...
vector<custom_priority_queue<JOB>> rq(NO_CORES);
...
vector<int> current_level(NO_CORES, 1);
...
class NonShutDownableCore
{
    public:
        ...
        bool migratable(double cur_time, int migratedCore, JOB mig_job){
        ...
        void migrateJobs(double cur_time){
            ...
            for(int i = 0; i < NO_CORES; i++){
                if(i == coreNumber or shutdownable[i] or !migratable(i, mig_job)) // check for LO criticality also
                    continue;
            ...
            while(cur_time < time){
                ...
                if(!arrival){ 
                    ...
                    if(HIcount > 0)
                        doSlackWorks(cur_time, k, rq[coreNumber], cur_job, extra_time);
                    if(idle){
                        migrateJobs(cur_time);  // what if a new job arrives when core is shutdown; it could be migrated to other core
                        double procrast_slack = dpm(cur_time, rq[coreNumber], k);
                        if(procrast_slack > PROCRAST_THRESHOLD){
                            decision_point = min(cur_time + procrast_slack, findMinTask());
                            total_idle_time += decision_point - cur_time;
...
class ShutDownableCore
{
    public:
        ...
        void migrateJobs(double cur_time){
            ...
            bool flag = false;
            for(int i = 0; i < NO_CORES; i++){
                if(i == coreNumber or shutdownable[coreNumber] or !shutdownable[i] or !migratable(i, mig_job)) // check for LO criticality also
                    continue;
                migratedJobs[i].push_back(mig_job);
                flag = true;
                break;
            ...
            while(cur_time < time){
                ...
                if(!rq[coreNumber].empty()){
                    ...
                    if(freq > Foptimal_max){
                        migrateJobs(cur_time) // new freq can go below Foptimal_min
                    }
                    freq = calculateFrequency();
