...
void rate_monotonic(int time) {
    ...
    map<int, deque<JOB>> priority_queues; // Map of queues, where key is the period and value is the {deadline, {remain_time, task_id}}
    ...
    while(cur_time < time){
        ...
        if(!arrival){
            for (int j = 0; j < num_of_task; j++) {
                if ((cur_time - arrival_time[j]) % period[j] == 0) {
                    ...
                    priority_queues[period[j]].push_back(t);
                    cur_task_info[j]++;
                    if(cur_job_index != -1){
                        cur_job.preempted_time = cur_time;
                        priority_queues[cur_job.period].push_front(cur_job);
                        cur_job_index = -1;
                    }
                }
            }
        ...
        if (it != priority_queues.end()){
            cur_job = it->second.front();
            it->second.pop_front();
			cur_job_index = cur_job.job_index;

            if(it->second.empty()){
                priority_queues.erase(cur_job.period);
            }
