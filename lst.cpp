...
int findLST(int cur_time, unordered_map<int, JOB>& rq){
    int minSlack = INT_MAX;
    int lst_job;
    for (auto it = rq.begin(); it != rq.end(); it++) {
        int slack = it->second.deadline - cur_time - it->second.remain_time;
        if (slack < minSlack) {
            minSlack = slack;
            lst_job = it->first;
        }
        else if(slack == minSlack){
            if(it->second.preempted_time > rq[lst_job].preempted_time){
                lst_job = it->first;
            }
        }
    }
...
void lst(int time) {
    ...
    unordered_map<int, JOB> rq;  // key = job_id, value = {deadline, {remain_time, task_id}}
    ...
    while(cur_time < time) {
        ...
        if (!rq.empty()) {
            cur_job_index = findLST(cur_time, rq);
            cur_job = rq[cur_job_index];
            rq.erase(cur_job_index);
